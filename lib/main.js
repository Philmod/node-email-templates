...
var EmailTemplate = function(templateDirectory, defaults, done) {
  ...
  this.render = function(locals, templatePath, html, text, subject, stylesheet, bufferType, callback) {
    ...
    var htmlOpts = { source: html, locals: locals }
      ...
      , subjectOpts  = { source: subject, locals: locals }
    ...
    subjectOpts.filename = glob.sync(path.join(locals.templatePath, '*subject.*'))[0];
    cssOpts.filename = glob.sync(path.join(locals.templatePath, '*style.*'))[0];

    // Render all templates and stylesheets.
    async.map([htmlOpts, textOpts, subjectOpts, cssOpts], tm.render.bind(tm), function(err, results) {
      if (err) return callback(err)
      html = results[0]
      text = results[1]
      subject = results[2]
      stylesheet = results[3]
      ...
      if (isBuffer) {
        async.map([ new Buffer(html), new Buffer(text), new Buffer(subject) ], zlib[bufferType], function(err, buffers) {
          if (err) return callback(err)
          html = buffers[0]
          text = buffers[1]
          subject = buffers[2]
          callback(null, html, text, subject)
        })
      }else {
        callback(null, html, text, subject)
      }
  ...
  fs.stat(templateDirectory, function(err, stats) {
    ...
    if (!stats.isDirectory())
      ...
      var batchCheck = function() {
        ...
        if (isBuffer) {
          that.render.call(scopeInfo, locals, templatePath, null, null, null, null, bufferType, callback);
        } else {
          that.render.call(scopeInfo, locals, templatePath, null, null, null, null, callback);
        }
      };
      ...
      async.series([
        ...
        function(cb) {
          ...
          fs.readFile(scopeInfo.html, 'utf8', function(err, data) {
            ...
            scopeInfo.subject = glob.sync(templatePath + '/*subject.*')[0] || '';
        ...
        function(cb) {
          async.map([scopeInfo.text, scopeInfo.stylesheet, scopeInfo.subject], checkExists, function(err, results) {
            if (err) return callback(err);

            if (!results[0]) scopeInfo.text = false
            if (!results[1]) scopeInfo.stylesheet = false
            if (!results[2]) scopeInfo.subject = false

            cb();
          });
        },
        function(cb) {
          if (scopeInfo.subject) {
            fs.readFile(scopeInfo.subject, 'utf8', function(err, data) {
              if (err) return callback(err);
              if (data === '') return callback(scopeInfo.subject + ' was empty');
              scopeInfo.subject = data;
              cb();
            });
          } else {
            cb();
